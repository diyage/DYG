...
class Loss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        coord_mask = coord_mask.unsqueeze(-1).expand_as(target_tensor).bool()  # [n_batch, S, S] -> [n_batch, S, S, N]
        noobj_mask = noobj_mask.unsqueeze(-1).expand_as(target_tensor).bool()  # [n_batch, S, S] -> [n_batch, S, S, N]
...
class YoLoV1Loss(nn.Module):
    def __init__(self,
                 ...
                 obj_conf_weight: float = 1.0,
                 no_obj_conf_weight: float = 0.5,
                 class_weight: float = 1.0
                 ):
        # write by diyage, a little different from original one
        super().__init__()
        self.pos_weight = pos_weight
        self.obj_conf_weight = obj_conf_weight
        self.no_obj_conf_weight = no_obj_conf_weight
        self.class_weight = class_weight

    @staticmethod
    def __get_grid_and_grid_box_mask(output: torch.Tensor, target: torch.Tensor) -> tuple:
        obj_grid_mask = torch.zeros(size=output.shape)
        obj_grid_box_mask = torch.zeros(size=output.shape)
        no_obj_grid_box_mask = torch.zeros(size=output.shape)

        batch, r, c, channels = target.shape
        for i in range(batch):
            for j in range(r):
                for k in range(c):
                    if target[i, j, k, 4]:  # has object locate cell [i, j, k,..]
                        obj_grid_mask[i, j, k, :] = 1
                        obj_grid_box_mask[i, j, k, :] = 1

                    else:
                        no_obj_grid_box_mask[i, j, k, :] = 1

        return obj_grid_mask, obj_grid_box_mask, no_obj_grid_box_mask

    @staticmethod
    def __get_position_loss_mask(obj_grid_box_mask: torch.Tensor)-> torch.Tensor:
        '''

        :param obj_grid_box_mask:  batch * cell * cell * 30   (x, y, w, h, c4) (x, y, w, h, c9) (class1, class2, ...)
        :return: position_loss_mask

        '''
        mask = torch.zeros(size=obj_grid_box_mask.shape)

        mask[:, :, :, 0:4] = 1.0
        mask[:, :, :, 5:9] = 1.0
        return mask * obj_grid_box_mask

    @staticmethod
    def __get_obj_conf_loss_mask(obj_grid_box_mask: torch.Tensor)-> torch.Tensor:
        mask = torch.zeros(size=obj_grid_box_mask.shape)
        mask[:, :, :, 4] = 1.0
        mask[:, :, :, 9] = 1.0
        return mask * obj_grid_box_mask
    ...
    @staticmethod
    def __get_no_obj_conf_loss_mask(no_obj_grid_box_mask: torch.Tensor)-> torch.Tensor:
        mask = torch.zeros(size=no_obj_grid_box_mask.shape)
        mask[:, :, :, 4] = 1.0
        mask[:, :, :, 9] = 1.0
        return mask * no_obj_grid_box_mask

    @staticmethod
    def __get_class_loss_mask(obj_grid_mask: torch.Tensor)-> torch.Tensor:
        mask = torch.zeros(size=obj_grid_mask.shape)
        mask[:, :, :, 10:] = 1.0
        return mask * obj_grid_mask

    def forward(self, output: torch.Tensor, target: torch.Tensor) -> tuple:
        # be careful
        # two boxes predict one object( original one just use one of the two boxes)
